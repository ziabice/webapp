...
* e la converte in un oggetto DateTime.
* 
...
***/
class TextDateInputFilter2 extends InputFilter {
	const
		// Formati per la data
	...
	protected function checkValue($value) {
		...
		if (preg_match($this->regex, $value, $parts) == 1) {
			if (checkdate($parts['month'], $parts['day'], $parts['year'])) {
				$this->the_date = new \DateTime();
				$this->the_date->setDate($parts['year'], $parts['month'], $parts['day']);
